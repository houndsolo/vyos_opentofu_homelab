locals {
  ...
  vrf_bgp_commands = flatten([
    for vrf in local.role_vrfs : [
      ...
      #"set vrf name ${vrf.vrf} protocols bgp address-family ipv4-unicast redistribute connected route-map 'RM-LYLAT-SERVICE-CONNECTED-TO-BGP'",
      #"set vrf name ${vrf.vrf} protocols bgp address-family ipv4-unicast route-map vpn export 'RM-LYLAT-SERVICE-BGP-TO-LOCAL-VPN'",
      ...
      #"set vrf name ${vrf.vrf} protocols bgp address-family l2vpn-evpn advertise ipv4 unicast route-map 'RM-LYLAT-SERVICE-BGP-TO-EVPN'",
      "set vrf name ${vrf.vrf} protocols bgp address-family l2vpn-evpn rd '${cidrhost(var.fabric.defaults.bgp_system_as,var.node.id)}:${vrf.vni}'",
